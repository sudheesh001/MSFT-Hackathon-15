var Core = new function () {
    ...
    this.init = function () {
        ...
        if (canvas && canvas.getContext) {
            ...
            item=baditems[Math.floor(Math.random()*5)];
            animate();
        }
    };
    ...
    function animate() {
        ...
        for (i = 0; i < organisms.length; i++) {
            ...
            /*.drawImage(image, p.position.x, p.position.y, 40, 40);*/
            var image1 = new Image();
            image1.src = "./img/carrot.png";
            if (p.type == ORGANISM_ENEMY) {
                context.drawImage(image, p.position.x, p.position.y, 20,20);
            }
            if (p.type == ORGANISM_ENERGY)/* context.fillStyle = 'rgba( 0, 235, 190, ' + p.alpha + ' )';*/
            context.drawImage(image1, p.position.x, p.position.y, 20, 20);
